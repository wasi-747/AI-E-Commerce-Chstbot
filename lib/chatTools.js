...
export const browseProducts = tool({
  description:
    "Search and filter the product catalog. Returns up to 5 matching products.",
  ...
  execute: async ({ query }) => {
    await dbConnect();
    console.log("🎯 AI is searching with query:", query);
...
export const addToCart = tool({
  description:
    "Add a specific product in a specific size to the user's shopping cart. Check inventory first — if the size is out of stock, automatically log a stock request and return out_of_stock status.",
  inputSchema: z.object({
...
export const removeFromCart = tool({
  description:
    "Remove a specific item from the user's cart. Match by product name (fuzzy) and size.",
  inputSchema: z.object({
    productName: z
      .string()
  ...
  execute: async ({ productName, size, userId }) => {
    ...
    return {
      ...
      removedName,
      size,
      message: `Removed ${removedName} (Size ${size}) from your cart.`,
    };
  },
});
...
export const viewCart = tool({
  description:
    "Fetch and display the current contents of the user's shopping cart.",
  inputSchema: z.object({}),
...
export const checkout = tool({
  description:
    "Place the user's order. Creates an Order document with item snapshots, clears the cart, and returns the order confirmation details.",
  inputSchema: z.object({}),
  execute: async ({ userId }) => {
    await dbConnect();
